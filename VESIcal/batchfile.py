...
class status_bar(object):
    ...
    def status_bar(percent, sample_name=None, btext=None, barLen=20):
        ...
        if sample_name is not None:
            sys.stdout.write("  Working on sample {0}              "
...
class BatchFile(object):
    ...
    ----------
        filename: str
            Path to the batch file, e.g., "my_file.xlsx".
            This always needs to be passed, even if the user is passing
            a pandas DataFrame rather than an batch file.
            If passing a DataFrame, filename should be set to None.
            File can be excel file (.xlsx) or .csv.
        ...
        sheet_name: str
            ...
            read_excel() sheet_name parameter. But functionality to read
            in more than one sheet at a time
            (e.g., pandas.read_excel(sheet_name=None))
            is not yet imlpemented in VESIcal.
            From the pandas 1.0.4 documentation:
        ...
        file_type: str
            ...
            extension .xlsx. Other option is 'csv', which denotes that
            the passed file has extension .csv.

        units: str
            OPTIONAL. Default is 'wtpt_oxides'. String defining whether the
            oxide composition is given in wt percent
            ("wtpt_oxides", which is the default), mole oxides (mol_oxides)
            or mole cations (mol_cations).
        ...
        default_normalization:     None or str
            ...
            including volatiles.
            The volatile wt% will remain fixed, whilst the other major element
            oxides are reduced proportionally so that the total is 100 wt%.
            - 'additionalvolatiles': Normalises major element oxide wt% to
            100%, assuming it is volatile-free. If H2O or CO2 are passed
            to the function, their un-normalized values will be retained
            in addition to the normalized non-volatile oxides,
            summing to >100%.
        ...
        label: str
            OPTIONAL. Default is 'Label'. Name of the column within the
            passed file referring to sample names.

        dataframe: pandas DataFrame
            OPTIONAL. Default is None in which case this argument is ignored.
            This argument is used when the user wishes to turn
            a pandas DataFrame into an BatchFile object, for example when user
            data is already in python rather than being imported
            from a file. In this case set `dataframe` equal to the dataframe
            object being passed in. If using this option, pass
            None to filename.
    """
    ...
    def __init__(self, filename, sheet_name=0, file_type='excel',
        ...
        if dataframe is not None:
            ...
            if file_type == 'excel':
                ...
                raise core.InputError("file_type must be one of "
                                      "\'excel\' or \'csv\'.")
        ...
        if 'model' in kwargs:
            w.warn("You don't need to pass a model here, so it will be "
                   ...
                   "calculations on your dataset "
                   "(e.g., calculate_dissolved_volatiles())",
                   RuntimeWarning, stacklevel=2)

        if 'norm' in kwargs:
            w.warn("We noticed you passed a norm argument here. "
                   "This does nothing. You can normalize your BatchFile and "
                   "save it to a new variable name after import using "
                   "normalize(BatchFileObject). "
                   "See the documentation for more info.",
                   RuntimeWarning, stacklevel=2)

        total_iron_columns = ["FeOt", "FeOT", "FeOtot", "FeOtotal",
                              "FeOstar", "FeO*"]
        for name in total_iron_columns:
            if name in data.columns:
                if 'FeO' in data.columns:
                    for row in data.itertuples():
                        if ((data.at[row.Index, "FeO"] == 0
                             and data.at[row.Index, name] > 0)):
                            ...
                            data.at[row.Index, "FeO"] = data.at[row.Index,
                                                                name]
                else:
                    w.warn("Total iron column " + str(name) + " detected. "
                           "This column will be treated as FeO. "
                           "If Fe2O3 data are not given, Fe2O3 will be 0.0. "
                           "In future, an option to calculate FeO/Fe2O3 "
                           "based on fO2 will be implemented.",
                           RuntimeWarning, stacklevel=2)
                    data['FeO'] = data[name]
        ...
        # data = data.where(data[core.oxides] > 0, 0)
        # Check for any negative oxide values, set to 0
        for column in data:
    ...
    def set_default_normalization(self, default_normalization):
        ...
        default_normalization:    str
            ...
            - 'standard' (default): Normalizes an input composition to 100%.
            - 'fixedvolatiles': Normalizes major element oxides to 100 wt%,
            including volatiles. The volatile wt% will remain fixed, whilst the
            other major element oxides are reduced
            proportionally so that the total is 100 wt%.
            - 'additionalvolatiles': Normalises major element oxide
            wt% to 100%, assuming it is volatile-free. If H2O or CO2 are passed
            to the function, their un-normalized values will
            be retained in addition to the normalized non-volatile oxides,
            summing to >100%.
    ...
    def get_composition(self, species=None, normalization=None, units=None,
        ...
        species:    NoneType or str
            The name of the oxide or cation to return the concentration of.
            If NoneType (default) the whole composition of each sample will
            be returned. If an oxide is passed, the value in
            wtpt will be returned unless units is set to 'mol_oxides',
            even if the default units for the sample object are mol_oxides.
            If an element is passed, the concentration will be returned as
            mol_cations, unless 'mol_singleO' is specified as units, even if
            the default units for the sample object are mol_singleO.
            Unless normalization is specified in the method call, none
            will be applied.
        ...
        normalization:     NoneType or str
            ...
            - 'standard' (default): Normalizes an input composition to 100%.
            - 'fixedvolatiles': Normalizes major element oxides to 100 wt%,
            including volatiles. The volatile wt% will remain fixed,
            whilst the other major element oxides are reduced
            proportionally so that the total is 100 wt%.
            - 'additionalvolatiles': Normalises major element oxide
            wt% to 100%, assuming it is volatile-free. If H2O or CO2 are passed
            to the function, their un-normalized values will be retained in
        ...
        exclude_volatiles   bool
            ...
            prior to normalization and
            conversion.

        asBatchFile:    bool
            If True, returns a BatchFile object.
            If False, returns a pandas.DataFrame object.
        ...
        if isinstance(new_compositions[0], pd.Series):
            return_frame = pd.concat([pd.DataFrame(j)
                                      for j in new_compositions], axis=1)
            ...
            return_frame = pd.DataFrame(species_data, index=[name for name
                                                             in sample_names])
        else:
            return_frame = None
    ...
    def get_data(self, normalization=None, units=None, asBatchFile=False):
        """
        Returns all data stored in a BatchFile object
        (both compositional and other data). To return only the
        ...
        non_compositional_data = data.filter([col for col in data.columns
                                              if col not in core.oxides])

        # concatenate both compositional and
        # non-compositional dataframes into one
    ...
    def get_sample_composition(self, samplename, species=None,
                               ...
                               asSampleClass=False):
        """
        Returns oxide composition of a single sample from a
        user-imported file as a dictionary
        ...
        asSampleClass:  bool
            If True, the sample composition will be returned as a
            sample class, with default options. In this case
        ...
        # Get sample composition in terms of any species, units,
    ...
    def try_set_index(self, dataframe, label):
        """
        Method to handle setting the index column in an BatchFile object.
        If no column is passed that matches the default index name,
        then this method will attempt to choose the 'best' column
        that the user might want to serve as an index column.
        ...
        label: str
            Name of the column within the passed Excel file
            referring to sample names.
        """
    ...
    def save_excel(self, filename, calculations, sheet_names=None):
        """
        Saves data calculated by the user in batch processing mode
        (using the BatchFile class methods) to an organized
        ...
        calculations: pandas DataFrame or list of pandas DataFrames
            ...
            and calculate_saturation_pressure). If None,
        ...
        if isinstance(calculations, list):
            if isinstance(sheet_names, list) or sheet_names is None:
                ...
                raise core.InputError("If calculations is passed as list, "
                                      "sheet_names must also be list of "
                                      "same length")
    ...
    def save_csv(self, filenames, calculations, **kwargs):
        ...
        comma-separated values (csv) file. Mirros the pandas.to_csv()
        method. Any argument that can be passed to pandas.csv() can be passed
        here. One csv file will be saved for each calculation
        passed.
        ...
        if len(filenames) != len(calculations):
            raise core.InputError("calculations and filenames must have "
                                  "the same length")
...
def from_DataFrame(dataframe, units='wtpt_oxides', label='Label'):
    ...
    units: str
        ...
        composition is given in wt percent ("wtpt_oxides",
        which is the default), mole fraction oxides ("mol_oxides"),
        or mole fraction cations ("mol_cations").

    label: str
        OPTIONAL. Default is 'Label'. Name of the column within the passed file
        referring to sample names. This
    ...
    """
    return BatchFile(filename=None, dataframe=dataframe,
